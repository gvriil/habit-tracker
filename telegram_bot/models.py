...
class TelegramState(models.Model):
    ...
    user = models.ForeignKey(settings.AUTH_USER_MODEL, verbose_name=_('Пользователь'),
                             on_delete=models.CASCADE, related_name='telegram_states')
...
class NotificationLog(models.Model):
    ...
    habit = models.ForeignKey('habits.Habit', verbose_name=_('Привычка'),
    ...
    user_response = models.CharField(_('Ответ пользователя'), max_length=50, blank=True, null=True)
...
class Achievement(models.Model):
    ...
    image = models.CharField(_('Эмодзи/Иконка'), max_length=50)  # Эмодзи для отображения в Telegram
...
class UserAchievement(models.Model):
    ...
    achievement = models.ForeignKey(Achievement, verbose_name=_('Достижение'),
