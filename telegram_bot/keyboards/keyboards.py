...
def get_habits_list_keyboard(habits):
    ...
    buttons.append([InlineKeyboardButton(text="➕ Добавить привычку", callback_data="new_habit")])
...
def get_habit_detail_keyboard(habit_id):
    ...
    buttons = [
        [InlineKeyboardButton(text="✅ Отметить выполнение", callback_data=f"complete_{habit_id}")],
...
def get_habit_notification_keyboard(habit_id):
    ...
    buttons = [
        ...
        [InlineKeyboardButton(text="⏰ Напомнить через 30 мин", callback_data=f"remind_{habit_id}")]
    ]
    return InlineKeyboardMarkup(inline_keyboard=buttons)
...
def get_frequency_keyboard():
    ...
    buttons = [
        ...
        [InlineKeyboardButton(text="📅 По будням (Пн-Пт)", callback_data="freq_weekdays")],
        [InlineKeyboardButton(text="🏖️ По выходным (Сб-Вс)", callback_data="freq_weekends")],
        [InlineKeyboardButton(text="📆 Выбрать дни недели", callback_data="freq_custom")]
    ]
    return InlineKeyboardMarkup(inline_keyboard=buttons)
...
def get_time_keyboard():
    ...
    buttons = [
        ...
        [InlineKeyboardButton(text="⏰ Указать точное время", callback_data="time_custom")]
    ]
    return InlineKeyboardMarkup(inline_keyboard=buttons)
...
def get_habit_action_keyboard(habit_id):
    ...
    buttons = [
        [
            InlineKeyboardButton(text="✅ Выполнить", callback_data=f"complete_{habit_id}"),
            InlineKeyboardButton(text="📊 Статистика", callback_data=f"stats_{habit_id}")
        ],
        [
