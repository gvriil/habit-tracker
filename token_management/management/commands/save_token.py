    ...
    def handle(self, *args, **kwargs):
        ...
        try:
            ...
            if "access" in token_data:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Access токен сохранен для пользователя {user_id}"
                    )
                )

            if "refresh" in token_data:
                token_service.store_user_token(
                    user_id, token_data["refresh"], "refresh"
                )
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Refresh токен сохранен для пользователя {user_id}"
                    )
                )
